 ...
 <div class="sl-mainpanel">
    ...
    <div class="sl-pagebody">
      <div class="sl-page-title">
        <h5>Subscriber Liste</h5>
      </div><!-- sl-page-title -->

      <div class="card pd-20 pd-sm-40">
        <h6 class="card-body-title">Subscriber Liste
        ...
        <div class="table-wrapper">
          <table id="datatable1" class="table display responsive nowrap">
            ...
            <tbody>
                @foreach ($sub as $key=>$rows)
              <tr>
                <td> <input type="checkbox"> {{ $key +1 }}</td>
                <td>{{ $rows->email }}</td>
                <td>{{ \Carbon\Carbon::parse($rows->created_at)->diffForhumans() }}</td>
                <td>
                    <a href="{{ URL::to("delete/sub/".$rows->id) }}" class="btn btn-sm btn-danger" id="delete">Supprimer</a>
                </td>
              </tr>
              @endforeach
